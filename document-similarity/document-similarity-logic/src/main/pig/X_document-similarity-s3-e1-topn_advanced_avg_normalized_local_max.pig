...
leftSim = LOAD '$outputPath$SIMILARITY_ALL_DOCS_SUBDIR' 
	as (docIdA:chararray,docIdB:chararray,sim:float);

gr = group leftSim by docIdA;
leftSimXA = foreach gr generate flatten(gr) 
	as (docIdA:chararray,docIdB:chararray,simX:float),  
	MAX(leftSim.sim) as max;
leftSimXB = foreach leftSimXA generate docIdA, docIdB, simX/max as sim;

STORE leftSimXB INTO '$outputPath$SIMILARITY_NORMALIZED_ALL_DOCS_SUBDIR';
