...
time pig -p tmpCompressionCodec=lzo -p tfidfMinValue=0.25 -p sample=0.1 -p commonJarsPath=${JARS} -p parallel=${PARALLEL} -p inputPath=full/hbase-dump/mproto-m* -p outputPath=${OUTPUT_PATH} -p mapredChildJavaOpts=-Xmx10000m -f ${SCRIPT_TFIDF}
