...
nohup <- read.delim("~/nohup.xpdendek", header=F)

nohupcount.csv <- read.delim("~/nohupcount.csv", header=F)
max(nohupcount.csv)
...
########################################################################
# with X^0 
ciemniak <- function(x){-4.9645056 + 0.0668870*x  + 0.0005803*x^2}
hadoop   <- function(x){2.921e+05  + -5.852e+02*x + 5.897e-01*x^2}
ciemniak(30000)
hadoop(30000)
# without X^0
ciemniakWO <- function(x){0.0608099*x +  0.0005811*x^2  }
hadoopWO   <- function(x){-0.4313334*x +   0.0005838*x^2 }
ciemniakWO(30000)
hadoopWO(30000)
# ciemniakWO(35000)/60
hadoopWO(35000)/60
...
plot(x,y
     ,xlim=range(0,7000)
#       ,ylim=range(10000,30000)
     ,ann=F
     )
# lines(x,yyy0(x),col="red")
lines(x,ciemniakWO(x),col="red")
lines(x,hadoopWO(x),col="blue")
