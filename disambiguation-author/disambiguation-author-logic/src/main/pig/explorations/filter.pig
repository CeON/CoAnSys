...
A1 = LOAD '$dc_m_hdfs_inputDocsData' as (sname:int, datagroup:{(cId:chararray, sname:int, data:map[{(int)}])}, count:long);
A2 = FILTER A1 by count <= 6627;
store A2 into '$dc_m_hdfs_outputContribs';
