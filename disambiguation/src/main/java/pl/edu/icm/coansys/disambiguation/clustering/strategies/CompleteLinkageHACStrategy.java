...
import pl.edu.icm.coansys.disambiguation.auxil.UniquePriorityQueue;
...
public abstract class CompleteLinkageHACStrategy implements ClusteringStrategy {
	...
	private Comparator<ClusterElement> mainComparator = new Comparator<ClusterElement>() {
		@Override
		public int compare(ClusterElement r1, ClusterElement r2) {
	 ...
	 * 
	 * @param similarities
	 *            A lower triangular matrix contains affinities between objects,
	 *            where a positive value inclines similarity and a negative
	 *            value reflects dissimilarity level.
	 * @return An array containing cluster numbers assigned to objects. Two
	 *         objects sharing the same cluster number may be considered as the
	 *         same one.
	 * @throws Exception
	 */
	@Override
	public int[] clusterize(float sim[][]) {
		...
		if (sim == null) {
			throw new IllegalArgumentException(
					"You are kindly asked to provide similarity matrix that exists");
		}
		if (sim.length == 1) {
			return new int[] { 0 };
		}
		...
		PriorityQueue<ClusterElement> P[] = new UniquePriorityQueue[sim.length];
		...
		for (int n = 0; n < sim.length; n++) {
			...
			// NlogN
			if (n != 0) {
				P[n] = new UniquePriorityQueue<ClusterElement>(4, mainComparator);
...
//			Debug code: use when needed
//			float[][] simBefore = new float[I.length][I.length];
//			System.out.println("");
//			for (int i = 1; i < P.length; i++) {
//				if (I[i] != 1) { continue; }
//				if (i == i1) { continue; }
//				for (ClusterElement ce : P[i]) {
//					int z1 = i;
//					int z2 = ce.getIndex();
//					simBefore[z1][z2] = ce.getSim();
//					simBefore[z2][z1] = ce.getSim();
//				}
//			}
			...
			System.out.println(rp.a + "," + rp.b);
			i1 = rp.a;
			i2 = rp.b;

			if (i1 == i2) {
				throw new InternalError(
						"Self-similarity detected! "
...
//			float[][] simNow = new float[I.length][I.length];
			...
			for (int i = 0; i < P.length; i++) {
				...
				if (i1 > i) {
					ClusterElement val = c_i_i1_recalc(C, i, i1, i2);
					P[i1].add(val);
				} else {
...
//					int z1 = i;
//					int z2 = ce.getIndex();
//					simNow[z1][z2] = ce.getSim();
//					simNow[z2][z1] = ce.getSim();
//				}
			}

//				Debug code: use when needed
//			System.out.println("--------------SimBefore--------------");
//			printTable(simBefore, I);
//			System.out.println("--------------SimNow--------------");
//			printTable(simNow, I);
//			System.out.println("==================================");
		}

		RankedUnionFindPathCompressed rufpc = new RankedUnionFindPathCompressed(
				I.length);
...
//	private void printPQ(PriorityQueue<ClusterElement> pq) {
//		for (ClusterElement ce : pq) {
//			System.out.print(ce + " ");
//		}
//		System.out.println();
//	}
//
//	private void printTable(float[][] table, int[] I) {
//		for (int a = 0; a < I.length; a++) {
//			for (int b = 0; b < I.length && b < a; b++) {
//				System.out.print(" "
//						+ Strings.padEnd(Integer.toString((int) table[a][b]),
//								5, ' ') + ";");
//			}
//			System.out.println();
//		}
//	}
	...
	private ClusterElement c_i_i1_recalc(ClusterElement[][] C, int i, int i1,
			int i2) {
		ClusterElement el;
		if (i1 > i && i2 > i) {
			el = C[i1][i];
	...
	protected RelaxedPair argMaxSequenceIndexExcludeSame(
			@SuppressWarnings("rawtypes") PriorityQueue[] priorityQueue, int[] I) {
		...
		RelaxedPair rp = maxTmp_index > maxEl_index ? new RelaxedPair(
				maxTmp_index, maxEl_index) : new RelaxedPair(maxEl_index,
				maxTmp_index);
