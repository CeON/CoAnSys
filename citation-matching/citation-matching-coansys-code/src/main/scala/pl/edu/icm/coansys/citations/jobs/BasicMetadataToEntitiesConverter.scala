...
import pl.edu.icm.coansys.citations.converters.BasicMetadataToEntityConverter
...
object BasicMetadataToEntitiesConverter extends MyScoobiApp {
  ...
  val basicMetadataToEntityConverter = new BasicMetadataToEntityConverter() 
  ...
  def run() {
    ...
    val entities = fromSequenceFile[String, BasicMetadata](inUri)
      .map {
      case (id, meta) => basicMetadataToEntityConverter.convert(id, meta)
    }
