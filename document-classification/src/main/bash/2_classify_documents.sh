...
eval "pig -x mapred -p DEF_SRC=${TABLE_TO_CLASSIFY} -p inMo=${MODEL_HDFS} -p DEF_DST=${RESULT_HDFS} -p featurevector=${FEATURE_VECTOR} -p simmeth=${SIMILARITY_METRIC} -p neigh=${NUMBER_OF_DOC_NEIGHBOURS} 2_MODEL_USE_classify.pig"
