...
%DEFAULT problematic_full /tmp/problematic_full
...
--describe X20;
X21 = foreach X20 generate FLATTEN(REGEX_EXTRACT_ALL($0,'([A-Z\\_]+)(.+)')) as (pre:chararray, key:chararray), flatten(documentMetaExtractor($1));
X211 = foreach X21 generate *,1 as crosspoint;

P = load '/user/pdendek/20130509/onetest/problematic/part-m-00000' as (problematic:chararray);
P1 = foreach P generate *,1 as crosspoint;
P1X211 = join X211 by crosspoint, P1 by crosspoint using 'replicated';
PX22 = filter P1X211 by key==problematic;
store PX22 into '$problematic_full';
