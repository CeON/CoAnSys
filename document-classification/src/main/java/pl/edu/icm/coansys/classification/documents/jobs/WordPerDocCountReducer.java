...
public class WordPerDocCountReducer extends Reducer<Text, StringListIntListWritable, TextArrayWritable, StringListIntListWritable> {
    ...
    @Override
    /**
     * (IN) accepts key-value pairs containing K:docId, V: word + its number of occurences in the document
     * (OUT) emit key-value pairs containing K:docId + word, V: the number of word occurences in the document + no. of all words in doc  
     */
