 ...
 */
package pl.edu.icm.coansys.classification.documents.pig.serializers;
...
import pl.edu.icm.coansys.models.DocumentClassificationProtos.DocumentClassificationOut;
...
public class SERIALIZE_RESULTS extends EvalFunc<Tuple> {
	...
	public Tuple exec(Tuple input) throws IOException {
		...
		try {
			...
			DataBag classifCodes = (DataBag) input.get(1);
			
			DocumentClassificationOut.Builder outb = DocumentClassificationOut.newBuilder();
			outb.setDocId(docId);
			
			for(Tuple bagTuple : classifCodes){
				outb.addClassificationCodes((String)bagTuple.get(0));
			}
