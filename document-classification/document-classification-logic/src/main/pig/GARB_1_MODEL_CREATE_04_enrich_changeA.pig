...
A1 = LOAD '$dc_m_hdfs_docClassifMapping' as (key:chararray,categs:bag{(categ:chararray)}); --key,{categ}
A2 = foreach A1 generate key;
A3 = group A2 all;
A4 = foreach A3 generate A2, 1 as crosspoint;
***********/

B1 = LOAD '$dc_m_hdfs_neighs' as (key:chararray,data:map[],part:int);
B2 = foreach B1 generate key,data;
B3 = $dc_m_pigScript_featureVector(B2);
B40 = group B3 by key;
%DEFAULT midproduct 'mid'
store B40 into '$dc_m_hdfs_dataEnriched$midproduct';

B4 = load '$dc_m_hdfs_dataEnriched$midproduct';
...
store G30 into store B40 into '$dc_m_hdfs_dataEnriched$lateproduct';

G3 = load '$dc_m_hdfs_dataEnriched$lateproduct';
